  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
         <div>
            <h1 className="text-3xl font-black text-[#0F172A] dark:text-white mb-1 tracking-tight">{t('clientsList.title')}</h1>
            <p className="text-[#526e9c] text-sm font-medium">{t('clientsList.subtitle')}</p>
         </div>
             <button
                  type="button"
                  onClick={() => {
             ...
             >
            <UserPlus size={18} /> {editingClientId ? t('clientsList.cancelEdit') : t('clientsList.newClient')}
         </button>
      </div>
         ...
         {showCreateForm && (
            ...
            >
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input
                     type="text"
                     placeholder={t('clientsList.namePlaceholder')}
                     value={form.nom}
                  ...
                  <input
                     type="email"
                     placeholder={t('clientsList.emailPlaceholder')}
                     value={form.email}
                  ...
                  <input
                     type="text"
                     placeholder={t('clientsList.phonePlaceholder')}
                  ...
                  <input
                     type="text"
                     placeholder={t('clientsList.addressPlaceholder')}
                     value={form.adresse}
               ...
               <div className="mt-4 flex justify-end gap-3">
                  ...
                  >
                     {isSaving && <Loader2 size={16} className="animate-spin" />} {editingClientId ? t('clientsList.update') : t('clientsList.add')}
                  </button>
               </div>
            </form>
         )}
        ...
        <div className="overflow-x-auto flex-1 flex flex-col">
          {isLoading ? (
             ...
             <div className="flex-1 flex items-center justify-center">
                <EmptyState 
                   icon={Users} 
                   title={t('clientsList.emptyTitle')} 
                   description={t('clientsList.emptyDesc')} 
                   actionText={t('clientsList.emptyAction')}
                   actionIcon={UserPlus}
                />
             </div>
          ) : (
          <table className="w-full text-left whitespace-nowrap min-w-[1000px]">
             <thead>
                <tr className="bg-[#526e9c]/5 text-[11px] uppercase tracking-widest text-[#526e9c] border-b border-[#526e9c]/20">
                   <th className="px-6 py-4 font-bold">{t('clientsList.company')}</th>
                   <th className="px-6 py-4 font-bold">{t('clientsList.email')}</th>
                   <th className="px-6 py-4 font-bold">{t('clientsList.phone')}</th>
                   <th className="px-6 py-4 font-bold">{t('clientsList.address')}</th>
                   <th className="px-6 py-4 font-bold text-center">{t('clientsList.actions')}</th>
                </tr>
             </thead>
                   ...
                   <tr key={cli.id} className="hover:bg-[#526e9c]/5 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                         <div className="flex justify-center gap-2">
                            ...
                            }} className="p-2 text-[#526e9c] hover:bg-[#18adf2]/10 hover:text-[#18adf2] rounded-lg transition-colors inline-block" title={t('clientsList.editTooltip')}>
                               <Edit2 size={20} />
                            </button>
                            <button onClick={() => { if(window.confirm(t('clientsList.deleteConfirm'))) deleteClient(cli.id) }} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors inline-block" title={t('clientsList.deleteTooltip')}>
                               <Trash2 size={20} />
                            </button>
                         </div>
                      </td>
